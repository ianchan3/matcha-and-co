...
import { webhookEvents, webhookFailures, ENV_LABEL } from "../metrics.js";
...
export async function stripeWebhook(req, res) {
  ...
  try {
    ...
    webhookEvents.add(1, { 'event.type': event.type, ...ENV_LABEL });
    ...
    webhookFailures.add(1, ENV_LABEL);
  ...
  if (event.type === "checkout.session.completed") {
    ...
    try {
      ...
      if (!existing) {
        ...
        const receiptNumber = paymentIntent.latest_charge?.receipt_number;
      ...
      if (err.code === 11000) {
        console.log("ℹ️ Duplicate webhook event ignored for session:", session.id);
